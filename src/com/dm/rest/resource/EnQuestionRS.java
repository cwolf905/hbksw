...
/**
 * 实现招生问题的已处理列表和统计、未处理列表和统计、信息查看、答复、审核，已经完成
 * 
 * @author Anthony
 * 
 */
...
public class EnQuestionRS extends BasicRS {
	...
	String filter, @RequestParam(value = "t", required = false)
		...
		return restTemplate
				.getForObject(
						RSCache.getURI()
								...
								+ encodeStr(filter) + "&t=" + t + "&status="
								+ status, String.class, teacherId, role);
	}
		...
		return restTemplate
				.getForObject(
						RSCache.getURI()
								...
								+ encodeStr(filter) + "&t=" + t, String.class,
						cid, teacherId, role);
	}
